        ...
        for groove_dim in range(4):
            ...
            if dataset_tag not in datasets:
                datasets[dataset_tag] = {
                    ...
                    "Stream 1 Vs. Flat Out | Hits | Hamming": [],  # Stream 1 of Output's jaccard distance to flattened input       # added √
                    "Stream 2 Vs. Flat Out | Hits | Hamming": [],  # Stream 2 of Output's jaccard distance to flattened input       # added √
                    "Stream 3 Vs. Flat Out | Hits | Hamming": [],  # Stream 3 of Output's jaccard distance to flattened input       # added √
                }
            ...
            i_fo_hamming = np.round(hamming_distance(input_hvo[:, 0], flat_out_hvo[:, 0], normalize=False), 6)
            ...
            valid_indices = get_timesteps_where_either_is_1(input_hits, flat_out_hits)
            ...
            for streams_permuation in list_permutations(streams):
                ...
                accent_hamming_normalized = i_fo_accent_hamming * 32.0 / valid_indices.shape[0] if valid_indices.shape[0] > 0 else 0.0
                datasets[dataset_tag]["Flat Out Vs. Input | Accent | Hamming"].append(accent_hamming_normalized)     # max hamming is the number of steps where either has a hit. Dividing by 32 in the line above compresses the possible values
                datasets[dataset_tag]["Stream 1 Vs. Flat Out | Hits | Hamming"].append(np.round(hamming_distance(flat_out_hvo[:, 0], streams_permuation[0][:, 0], normalize=True), 6))
                datasets[dataset_tag]["Stream 2 Vs. Flat Out | Hits | Hamming"].append(np.round(hamming_distance(flat_out_hvo[:, 0], streams_permuation[1][:, 0], normalize=True), 6))
                datasets[dataset_tag]["Stream 3 Vs. Flat Out | Hits | Hamming"].append(np.round(hamming_distance(flat_out_hvo[:, 0], streams_permuation[2][:, 0], normalize=True), 6))
...
def compile_data_for_multiple_datasets_pkl(data_dir,  dataset_pkls, accent_v_thresh = 0.75):
    ...
    destination_dir = f"data/triple_streams/cached/AccentAt{accent_v_thresh}/separate_sets"
    os.makedirs(destination_dir, exist_ok=True)

    # save the dataset
    for key, value in dataset_dict.items():
        final_dict_fname = f"{key}.pkl.bz2" if "candombe" not in key else f"candombe_four_voices.pkl.bz2"
        print("Final dictionary filename will be:", final_dict_fname)

        compiled_data_path = os.path.join(destination_dir, f"{final_dict_fname}")
        with bz2.BZ2File(compiled_data_path, 'wb') as f:
            pickle.dump(value, f)
