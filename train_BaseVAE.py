...
if __name__ == "__main__":
    ...
    wandb_run = wandb.init(
        ...
        settings=wandb.Settings(code_dir="train_BaseVAE.py")    # for code saving
    )
    ...
    if loaded_via_config:
        ...
        model_code.add_file("train_BaseVAE.py")
    ...
    def predict_using_batch_data(batch_data, model_=model_on_device, device=config.device):
        flat_hvo_groove, _, outputs_, _, _, _, _, _ = batch_data_extractor(batch_data, device)
        with torch.no_grad():
    ...
    def forward_using_batch_data(batch_data, model_=model_on_device, device=config.device):
        flat_hvo_groove, _, target_outputs, _, _, _, _, _ = batch_data_extractor(batch_data, device)
