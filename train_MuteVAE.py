...
if __name__ == "__main__":
    ...
    if loaded_via_config:
        ...
        model_code.add_file("train_MuteVAE.py")
    ...
    def predict_using_batch_data(batch_data, model_=model_on_device, device=config.device):
        flat_hvo_groove, _, outputs_, kick_is_muted, snare_is_muted, hat_is_muted, tom_is_muted, cymbal_is_muted = batch_data_extractor(batch_data, device)
        with torch.no_grad():
    ...
    def forward_using_batch_data(batch_data, model_=model_on_device, device=config.device):
        flat_hvo_groove, _, target_outputs, kick_is_muted, snare_is_muted, hat_is_muted, tom_is_muted, cymbal_is_muted = batch_data_extractor(batch_data, device)
